# scripts/main/world_creation_forest_sliders_modal.gd
# Side panel shown during the "Let there be Forests" step — controls cluster size and how
# thin/branching ("tendril") vs. thick/compact the forest fingers grow, then redraws.
extends Control
...
var _tendril_slider: HSlider
var _size_value_lbl: Label
var _tendril_value_lbl: Label

func setup_integrated(world_creation_ref: Node, ui_layer: Node) -> void:
	_world_creation = world_creation_ref
	name = "ForestSlidersModal"
...
func _build_ui():
	...
	_size_value_lbl = Label.new()
	_build_slider_row(vbox, "Forest Size", _world_creation.forest_size_value, _size_slider, _size_value_lbl)

	_tendril_slider = HSlider.new()
	_tendril_value_lbl = Label.new()
	_build_slider_row(vbox, "Tendril Spread", _world_creation.forest_tendril_spread, _tendril_slider, _tendril_value_lbl)

	var tendril_hint = Label.new()
	tendril_hint.text = "Compact  ⟷  Slender Tendrils"
	tendril_hint.add_theme_font_size_override("font_size", 10)
	tendril_hint.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	tendril_hint.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	vbox.add_child(tendril_hint)
...
func _on_regenerate_pressed():
	if is_instance_valid(_world_creation):
		_world_creation.regenerate_forest_from_sliders(_size_slider.value, _tendril_slider.value)
